...
def main(input_dir, verbose, output):
    ...
    if output == "html":
        generated_dest = f"{input_dir}/html"
    else:
    ...
    if os.path.exists(f"{input_dir}/static"):
        ...
        for static_file in static_files:
            full_file_name = os.path.join(f"{input_dir}/static")
    ...
    with open(f"{input_dir}/config.json") as js:
        data = json.load(js)
        ...
        for d in data:
            ...
            generated = template.render(d["context"])

            if not os.path.exists(f"{generated_dest}{url}"):
                os.makedirs(f"{generated_dest}{url}")

            with open(f"{generated_dest}{url}/index.html", "w") as f:
                f.write(generated)
