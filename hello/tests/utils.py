...
def assert_config_not_modified(input_dir):
    ...
    original_config = (TESTDATA_DIR/input_dir/"config.json").resolve()
    student_config = (input_dir/"config.json").resolve()
    assert student_config.exists()
    assert original_config.exists()
    assert student_config != original_config
    assert filecmp.cmp(student_config, original_config), (
        "Modified config file.  These files do not match:\n"
